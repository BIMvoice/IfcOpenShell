...
void svg_serializer::apply_settings() {
	...
	svg_render_hidden_edges_ = settings().get<ifcopenshell::geom::settings::SvgRenderHiddenEdges>().get();
}
	...
	for (auto sit = section_heights_used->begin(); sit != section_heights_used->end(); ++sit) {
		...
		if (use_hlr) { // && (hlr.which())) {
			...
			if (any_in_front && !data.product.declaration().is("IfcAnnotation") && !data.product.declaration().is("IfcSpace") && !data.product.declaration().is("IfcGrid")) {
				...
				// later extracted as (see hlr_calc::extract() in svg_serializer.h).
				//
				...
				if (is_floor_plan_) {
					if (storey) {
						...
						if (it == storey_hlr.end()) {
							it = storey_hlr.insert({ storey, hlr_engine(logger(), use_prefiltering_, use_hlr_poly_, segment_projection_, projection_plane, svg_render_hidden_edges_) }).first;
						}
...
void svg_serializer::finalize() {
	...
	if (deferred_section_data_ && deferred_section_data_->size() && element_buffer_.size()) {
		...
		for (auto& sd : *deferred_section_data_) {
			...
			if (use_hlr && pln) {
				hlr = new hlr_engine(logger(), use_prefiltering_, use_hlr_poly_, segment_projection_, *pln, svg_render_hidden_edges_);
			}
