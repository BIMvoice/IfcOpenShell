...
bool ifcopenshell::geom::util::boolean_operation(const boolean_settings& settings, const TopoDS_Shape& a_input, const NCollection_List<TopoDS_Shape>& b_input, BOPAlgo_Operation op, TopoDS_Shape& result, double fuzziness, bool heal_operand_on_failure) {
	...
	if (!success) {
		if (allow_retry) {
			...
			if (!a_healed.IsNull() && !a_healed.IsSame(a_input)) {
				settings.log().notice("GEO", 154, "Retrying cut with small edges removed from first operand");
