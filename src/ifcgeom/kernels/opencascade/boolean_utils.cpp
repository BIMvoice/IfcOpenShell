...
namespace ifcopenshell { namespace geom { namespace util {
...
bool ifcopenshell::geom::util::boolean_operation_impl(const boolean_settings& settings, const TopoDS_Shape& a_input, const NCollection_List<TopoDS_Shape>& b_input, BOPAlgo_Operation op, TopoDS_Shape& result, double fuzziness, bool allow_nonmanifold) {
				...
				if (!success) {
					...
					if (!success && allow_nonmanifold && op == BOPAlgo_CUT) {
						...
						if (va > 1.e-9 && (va - vr) > va * 1.e-4) {
							success = true;
							settings.log().notice("GEO", 157, "Accepting valid but non-manifold boolean CUT result as last resort");
						}
					}
				}
...
bool ifcopenshell::geom::util::boolean_operation(const boolean_settings& settings, const TopoDS_Shape& a, const NCollection_List<TopoDS_Shape>& b, BOPAlgo_Operation op, TopoDS_Shape& result, double fuzziness) {
	...
	if (b.Extent() > 1) {
		...
		if (any_applied) {
			settings.log().notice("GEO", 158, "Applied subtraction operands sequentially as last resort");
			result = current;
			return true;
		}
		return false;
