...
bool ifcopenshell::geom::util::boolean_operation(const boolean_settings& settings, const TopoDS_Shape& a_input, const NCollection_List<TopoDS_Shape>& b_input, BOPAlgo_Operation op, TopoDS_Shape& result, double fuzziness, bool enlarge_tools_on_failure) {
	...
	*/
	...
	if (!success) {
		if (allow_retry) {
			...
			if (any_enlarged) {
				settings.log().notice("GEO", 154, "Retrying cut with slightly enlarged tool operands");
