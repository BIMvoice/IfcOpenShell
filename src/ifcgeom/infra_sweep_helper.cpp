...
// a vertex list without dropping their curvature (and std::get_if<point3::ptr>
// would return null on a parameter-valued trim).
bool loop_is_polygonal(const taxonomy::loop::ptr& loop) {
	for (const auto& e : loop->children) {
		...
		if (std::get_if<taxonomy::point3::ptr>(&e->start) == nullptr ||
			std::get_if<taxonomy::point3::ptr>(&e->end) == nullptr) {
			return false;
		}
	}
	return true;
