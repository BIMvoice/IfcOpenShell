...
class Drawing(bonsai.core.tool.Drawing):
    ...
    @classmethod
    def is_generated_reference_annotation(cls, element: ifcopenshell.entity_instance) -> bool:
        """Whether ``element`` was created by sync_references' auto reference
        generation, as opposed to a same-typed annotation the user placed by
        hand through Add Annotation. Only the former is safe to auto-sync or
        auto-delete."""
        return bool(ifcopenshell.util.element.get_pset(element, "EPset_Annotation", "IsAutoGenerated"))

    @classmethod
    def mark_as_generated_reference_annotation(cls, element: ifcopenshell.entity_instance) -> None:
        ifc_file = tool.Ifc.get()
        pset = tool.Pset.get_element_pset(element, "EPset_Annotation")
        if not pset:
            pset = ifcopenshell.api.pset.add_pset(ifc_file, product=element, name="EPset_Annotation")
        ifcopenshell.api.pset.edit_pset(ifc_file, pset=pset, properties={"IsAutoGenerated": True})
