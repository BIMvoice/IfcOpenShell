...
def sync_external_styles_for_space(space: bpy.types.SpaceView3D, scene: Union[bpy.types.Scene, None]) -> None:
    """Keep material/external style state in sync with a 3D viewport's shading.

    Shared by the draw handler below (covers any viewport, including ones
    created mid-session, see #3296) and by call sites that want to force an
    immediate refresh for a known set of areas (e.g. after loading a project).
    """
...
def viewport_shading_changed_callback(area: bpy.types.Area) -> None:
    """Force-refresh style state for a single already-known area (e.g. right after project load)."""
    sync_external_styles_for_space(area.spaces.active, bpy.context.scene)


def sync_external_styles_on_viewport_draw() -> None:
    """Keep material/external style state in sync with the active 3D viewport's shading.
    ...
    runs on every viewport draw call; restore_material_style_types() itself
    is a no-op unless the shading type actually changed.
    """
    space = bpy.context.space_data
    if not isinstance(space, bpy.types.SpaceView3D):
        return
    sync_external_styles_for_space(space, bpy.context.scene)
